...
#take integers from 1 to 5, and multiply each by two
sapply(1:5, mult.2)
...
## use sapply() to apply a function to that generates more than just a vector
v=c(1,2,3)
sapply(1:5, function(x) x*v)
lapply(1:5, function(x) x*v)

### use sapply() and lapply() to run a linear model on multiple datasets
iris
...
spp.mods
summary(spp.mods[[1]]) #results for lm on setos
summary(spp.mods[[2]]) #results for lm on versicolor
summary(spp.mods[[3]]) #results for lm on virginia
...
### for-loops()
for(i in 1:5) print(i*2)   #i stands for iteration
...
sample(1:10, 10) #gives you 10 number between 1 and 10 in randomised order
sample(1:10, 10, replace=TRUE) #BOOTSTRAPPING=sampling with replacement, basically like picking number out of a hat, than replacing it back into the hat
...
for(i in 1:100) {
  ...
  }#i'm going to pick the cars assigned by "samp" and take the mean of mpg
